...
function updateShareCodesCDN(url = 'https://cdn.jsdelivr.net/gh/ZFeng3242/updateTeam@master/shareCodes/jd_cityShareCodes.json') {
  return new Promise(resolve => {
...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: `https://github.com/ZFeng3242/updateTeam/raw/master/shareCodes/jd_cityShareCodes.json`, 'timeout': 20000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
