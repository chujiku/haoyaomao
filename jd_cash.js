...
function readShareCode() {
  ...
  return new Promise(async resolve => {
    $.get({url: "https://raw.githubusercontent.com/ZFeng3242/RandomShareCode/main/JD_Cash.json",headers:{
      ...
      try {
        if (err) {
          ...
          $.get({url: `https://raw.githubusercontent.com/ZFeng3242/RandomShareCode/main/JD_Cash.json`, 'timeout': 10000},(err, resp, data)=>{
          data = JSON.parse(data);})
        } else {
          if (data) {
            console.log(`随机取助力码放到您固定的互助码后面(不影响已有固定互助)`)
...
function getAuthorShareCode() {
  return new Promise(resolve => {
    $.get({url: "https://raw.githubusercontent.com/ZFeng3242/updateTeam/master/shareCodes/jd_updateCash.json",headers:{
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1 Edg/87.0.4280.88"
      }}, async (err, resp, data) => {
      $.authorCode = [];
      try {
