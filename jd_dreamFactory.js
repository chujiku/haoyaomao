...
function updateTuanIds(url = 'https://raw.githubusercontent.com/ZFeng3242/updateTeam/master/shareCodes/jd_updateFactoryTuanId.json') {
  return new Promise(resolve => {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://raw.githubusercontent.com/ZFeng3242/updateTeam/master/shareCodes/jd_updateFactoryTuanId.json`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function requireConfig() {
  return new Promise(async resolve => {
    await updateTuanIdsCDN('https://raw.githubusercontent.com/ZFeng3242/updateTeam/master/shareCodes/jd_updateFactoryTuanId.json');
    if (!$.tuanIdS) await updateTuanIds();
    if (!$.tuanIdS) await updateTuanIdsCDN('https://cdn.jsdelivr.net/gh/ZFeng3242/updateTeam/master/shareCodes/jd_updateFactoryTuanId.json');
